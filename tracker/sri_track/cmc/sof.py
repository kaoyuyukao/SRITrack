...
class SparseOptFlow(CMCInterface):
    ...
    def apply(self, img, dets):
        ...
        try:
            ...
            # H, _ = cv2.estimateAffine(self.prev_keypoints, next_keypoints, cv2.RANSAC)
        except Exception as e:
        ...
        #H[:(H_h/3), :(H_w/3)]*=0.75
        #H[:(H_h/3), (H_w/3):(H_w/3)*2]*=0.8
        #H[:(H_h/3), (H_w/3)*2:H_w+1]*=0.75

        #H[(H_h/3):(H_h/3)*2, :(H_w/3)]*=0.8
        #H[(H_h/3):(H_h/3)*2, (H_w/3):(H_w/3)*2]*=0.9
        #H[(H_h/3):(H_h/3)*2, (H_w/3)*2:H_w+1]*=0.8

        #H[(H_h/3)*2:H_h, :(H_w/3)]*=0.9
        #H[(H_h/3)*2:H_h, (H_w/3):(H_w/3)*2]*=1
        #H[(H_h/3)*2:H_h, (H_w/3)*2:H_w+1]*=0.9
